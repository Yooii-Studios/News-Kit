 ...
 */
public class NLNewsFeedFetchUtil {
    private static final int MAX_DESCRIPTION_LENGTH = 200;
    private static final String ILLEGAL_CHARACTER_OBJ = Character.toString((char) 65532);

    public static NLNewsFeed fetch(Context context, NLNewsFeedUrl feedUrl,
                                   int fetchLimit) {

        if (!feedUrl.getType().equals(NLNewsFeedUrlType.GENERAL)) {
            // 디폴트 세팅을 사용할 경우 패널단에서 언어설정을 감지 못하므로 무조건 현재 언어의
            // 디폴트 url을 가져온다.
            feedUrl = NLNewsFeedUtils.getDefaultFeedUrl(context);
        }
