...
public class NaverIabManager extends IabManager {
    ...
    @Getter private NIAPHelper mHelper = null;
    ...
    private void queryProductsInfo() {
        // 전체 아이템의 정보를 불러옴
        mHelper.getProductDetailsAsync(IabProducts.makeProductKeyList(), new NIAPHelper.GetProductDetailsListener() {
            @Override
    ...
    private void queryPurchases() {
        ...
        mHelper.getPurchasesAsync(new NIAPHelper.GetPurchasesListener() {
            @Override
            public void onSuccess(List<com.naver.android.appstore.iap.Purchase> purchases) {
                if (isHelperDisposed()) {
                    return;
                }
                // 구매 목록 저장
    ...
    private Map<String, String> getPrices(List<Product> products) throws IabDetailNotFoundException {
        List<String> productCodes = IabProducts.makeProductKeyList();
                ...
                new NIAPHelper.RequestPaymentListener() {
                    ...
                    public void onSuccess(Purchase purchase) {
                        ...
                        boolean hasValidMd5 = purchase.getDeveloperPayload().equals(
                                Md5Utils.getMd5String(productCode));
                        if (hasValidMd5) {
                            String googleSku = NIAPUtils.convertToGoogleSku(productCode);

                            IabProducts.saveIabProduct(googleSku, mActivity.getApplicationContext());
